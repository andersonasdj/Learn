...
import br.com.techgold.learn.orm.ProjecaoContagemStatus;
...
public interface FuncionarioRepository extends JpaRepository<Funcionario, Long> {
	...
	@Query(value = "SELECT s.status AS status, COUNT(s.id) AS qtd FROM solicitacoes s "
			+ "WHERE s.funcionario_id=:funcionarioId AND s.excluido=0 "
			+ "AND s.status IN ('ABERTO','ANDAMENTO','AGENDADO','AGUARDANDO','PAUSADO') "
			+ "GROUP BY s.status", nativeQuery = true)
	public List<ProjecaoContagemStatus> buscaContagemPorFuncionario(Long funcionarioId);

	@Query(value = "SELECT s.status AS status, COUNT(s.id) AS qtd FROM solicitacoes s "
			+ "WHERE s.excluido=0 "
			+ "AND s.status IN ('ABERTO','ANDAMENTO','AGENDADO','AGUARDANDO','PAUSADO') "
			+ "GROUP BY s.status", nativeQuery = true)
	public List<ProjecaoContagemStatus> buscaContagemGeralAgrupada();
